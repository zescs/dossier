...
<body>
			...
			<tr>
			   <td style="width:100%;">   
                ...
                <td style="white-space:nowrap;">
                	<label class="common-label"><fmt:message key="archives.entry.message.retrieve.list.retrieveType"/>:</label>
                	<input id="retrieveType" 
                		   name="retrieveType"
                		   class="mini-combobox"
                		   errorMode="border"
                		   style="width:110px;"
                		   data='${r'${retrieveTypes}'}'
                		   emptyText="<fmt:message key="archives.entry.message.retrieve.list.retrieveType.emptyText"/>"
						   allowInput="false"
        			 	   valueField="id" 
        			 	   value="MATCHING"
						   textField="text" />
	...
	<div class="mini-fit">
		<div id="entityDG"
			...
			showEmptyText="true"
			url="<c:url value="/archives/entry/retrieve/list.action" />"
			idField="id"
			sizeList="${r'${sizeList}'}"  
    ...
    <ul id="gridMenu" class="mini-contextmenu" onbeforeopen="onBeforeOpen">              
       <li iconCls="icon-cus-folder_classic_opened_stuffed" onclick="onLookVolumes"><fmt:message key="archives.message.title.tab.volumes" /></li>
    ...
    <!-- contextmenu-end -->
    <div id="entityDetialForm" style="display:none;height:500px;">
       <div  class="mini-tabs" 
       		 id="entityTabs"
       		 style="width:100%;height:100%;"
       		 activeIndex="0"
            ...
            <div  title="<fmt:message key="archives.entry.note" />" name="defaultTab" iconCls="icon-cus-view_less_text">
       		...
       		<div  title="<fmt:message key="archives.message.title.tab.volumes" />" name="volumesTab" iconCls="icon-cus-document_letter_marked" refreshOnClick="true" url="<c:url value="/archives/entry/retrieve/volumes/list.action" />"></div>
       </div>
    </div>
	<!-- body-end -->
	<script type="text/javascript">
	  ...
	  function onShowRowDetail(e) {
         ...
         grid.loading();
         $.ajax({
              url: "<c:url value='/archives/entry/retrieve/getnote.action' />",
              type:"get",
              dataType:"json",
              cache: false,
              data:{"entryId":row.id},
	  ...
	  function onActionHandleRenderer(e){
	  	var grid = e.sender;
        var record = e.record;
        var s = '<a href="javascript:onEditElectFile(0);" class="btn_common"><fmt:message key="global.message.edit.electFile"/></a>';
        return s;
	  }
	  <#if existSwf>
	  ...
	  function complexSearch(){
		mini.open({
                url:'<c:url value="/archives/entry/retrieve/search.action" />?archivesId='+mini.get("archivesId").getValue(),
	  ...
	  function onLookVolumes(){
		...
		if(row){
			mini.open({
	                url:'<c:url	value="/archives/entry/retrieve/volumes/list.action" />?archivesId='+mini.get("archivesId").getValue()+"&entryId="+row.id,
	   		 ...
	   		 mini.showMessageBox({
		            ...
		            callback: function (action) {
		            	if(action=='ok'){
		            		...
		            		$.ajax({
		            			url:'<c:url value="/archives/entry/retrieve/remove.action" />',
      ...
      function search(){
		formData["keyword"]=mini.get("#keyword").getValue();
		formData["retrieveType"]=mini.get("#retrieveType").getValue();
		grid.load(formData);
	  }
	  ///////////////////////////////////////////
	</script>
