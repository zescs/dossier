...
<body>
			...
			<label class="common_label"></label>
	  		<div class="from_btn_middle">
	  			<div class="form_btn_left">
	  				<a class="mini-button" iconCls="icon-edit" onclick="onSaveData"><fmt:message key="global.message.edit"/></a>
	  			</div>
	...
	<input id="dh_stencil_data" value="${archivesNoConfigId}" class="mini-hidden" />
	</#if>
	<!-- hidden-end -->
	<!-- form-end -->
	<script type="text/javascript">
		...
		var tempData={};
		///////////////////////////////////////////
		function initData(_tempData){
			$.ajax({
                url: '<c:url value="/archives/entry/getEditById.action"/>',
                type:'post',
                data: {"id":_tempData.entryId},
                success: function (data) {
                	tempData=data;
                	setFromData(form,data,false);
                	<#if archivesNoConfigId?exists>
                	mini.get("dh_stencil").setValue(mini.get("dh_stencil_data").getValue());
                	</#if>
                },
                dataType:'json'
            });
		...
		function onSaveData() {
            ...
            data["_method"]="put";
            $.ajax({
                url: '<c:url  value="/archives/entry/edit.action" />',
				type: 'post',
                data: data,
                cache: false,
                dataType:'json',
